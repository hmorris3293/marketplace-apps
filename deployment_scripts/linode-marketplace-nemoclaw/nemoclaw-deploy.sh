...
## NemoClaw installer settings
#<UDF name="nemoclaw_install_ref" label="NemoClaw installer ref/tag" default="lkg" example="lkg or vX.Y.Z">
...
EOF
  ...
  if [[ -n ${NEMOCLAW_INSTALL_REF} ]]; then
    echo "nemoclaw_ref: ${NEMOCLAW_INSTALL_REF}" >> ${group_vars};
  else
    echo "nemoclaw_ref: lkg" >> ${group_vars};
  fi
